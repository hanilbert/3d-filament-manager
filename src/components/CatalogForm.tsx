...
export function CatalogForm({ initialValues, catalogId }: CatalogFormProps) {
  ...
  useEffect(() => {
    async function loadMaterials() {
      try {
        const data = await apiFetch<{ material: string }[]>("/api/filaments?groupBy=material");
        setMaterials(data.map((d) => d.material));
      } catch {
        // ignore
      }
    }
    ...
    async function loadBrands() {
      try {
        const data = await apiFetch<BrandOption[]>("/api/filaments?groupBy=brandList");
        setBrandList(data);
      } catch {
        // ignore
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (catalogId) {
        await apiFetch(`/api/filaments/${catalogId}`, {
          method: "PATCH",
          body: JSON.stringify(body),
        });
        router.push(`/filaments/${catalogId}`);
      } else {
        const created = await apiFetch<{ id: string }>("/api/filaments", {
          method: "POST",
          body: JSON.stringify(body),
        });
        router.push(`/filaments/${created.id}`);
      }
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="app-content space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <select
            id="material"
            value={allMaterialOptions.includes(values.material) ? values.material : values.material === "" ? "" : "__custom"}
            onChange={(e) => update("material", e.target.value === "__custom" ? "" : e.target.value)}
            className="w-full h-12 rounded-md border border-input bg-background px-3 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring appearance-none"
            required
          >
        ...
        {values.material !== "" && !allMaterialOptions.includes(values.material) && (
          <Input
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2 items-center">
          <Input id="color_hex" type="color" value={values.color_hex || "#ffffff"} onChange={(e) => update("color_hex", e.target.value)} className="w-14 h-12 p-1 cursor-pointer" />
          <Input value={values.color_hex} onChange={(e) => update("color_hex", e.target.value)} placeholder="#5C8A3C（可留空）" className="flex-1 h-12" />
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <Input type="file" accept=".jpg,.jpeg,.png,.webp" onChange={handleLogoUpload} disabled={uploading} className="h-12" />
