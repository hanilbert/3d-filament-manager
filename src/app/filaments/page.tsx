...
export default function FilamentsPage() {
  ...
  const [searchItems, setSearchItems] = useState<FilamentItem[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        if (isSearching) {
          const data = await apiFetch<FilamentItem[]>(`/api/filaments?q=${encodeURIComponent(q)}`);
          setSearchItems(data);
        } else {
          const data = await apiFetch<BrandGroup[]>("/api/filaments?groupBy=brand");
          setBrands(data);
        }
      } finally {
        setLoading(false);
                ...
                <Link key={item.id} href={`/filaments/${item.id}`}>
                  <div className="flex items-center gap-3 p-4 bg-card border border-border rounded-lg active:bg-muted transition-colors hover:bg-muted/50">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate">{[item.material, item.variant].filter(Boolean).join(" ")} · {item.color_name}</p>
                    </div>
...
function BrandTable({ brands }: { brands: BrandGroup[] }) {
  ...
  const materialCols = ["PLA", "PETG", "ABS", "ASA", "TPU"];
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border bg-muted/30">
            ...
            {materialCols.map((m) => (
              <th key={m} className="text-right px-3 py-3 font-medium text-muted-foreground whitespace-nowrap hidden lg:table-cell">{m}</th>
            ))}
        ...
        <tbody>
          {brands.map((b) => {
            const materialSet = new Set(b.materials);
            return (
              <tr key={b.brand} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                <td className="px-4 py-3">
                  <Link href={`/filaments/brand/${encodeURIComponent(b.brand)}`} className="flex items-center gap-2.5 hover:text-primary transition-colors">
                    {b.logo_url ? (
                ...
                {materialCols.map((m) => (
                  <td key={m} className="text-right px-3 py-3 tabular-nums text-muted-foreground hidden lg:table-cell">{materialSet.has(m) ? "·" : ""}</td>
                ))}
