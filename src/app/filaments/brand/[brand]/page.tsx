...
type SortField = "brand" | "material" | "variant" | "color_name" | "color_hex";
...
function normalizeHex(value?: string | null): string {
  return value && /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(value) ? value.toUpperCase() : "--";
}
  ...
  const groupedItems = useMemo<GroupedItems[]>(() => {
    ...
    for (const item of items) {
      ...
      if (existing) {
        ...
        groups.set(key, { key, material, variant, items: [item] });
      }
    }
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {items.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="border-b border-border/70 last:border-0 hover:bg-muted/30 cursor-pointer transition-colors" onClick={() => router.push(`/filaments/${item.id}`)}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <span className="inline-block w-24 h-8 rounded-md border border-border/60" style={{ backgroundColor: swatchColor }} />
              ...
              {groupedItems.map((group) => (
                <section key={group.key} className="border border-border rounded-lg overflow-hidden bg-card">
                  <div className="px-4 py-2.5 bg-muted/30 border-b border-border">
                    <p className="text-sm font-medium">{group.material}</p>
                    <p className="text-xs text-muted-foreground">{group.variant}</p>
                  </div>

                  <div>
                    {group.items.map((item) => (
                      <div key={item.id} className="px-4 py-3 border-b border-border/60 last:border-0 flex items-center gap-3">
                        <Link href={`/filaments/${item.id}`} className="flex-1 min-w-0">
